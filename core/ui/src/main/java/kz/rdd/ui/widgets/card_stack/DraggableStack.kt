...
open class CardStackController(
    ...
    val cardMainScale = Animatable(1f)
    ...
    private fun CoroutineScope.returnCenterNoAnim() {
        launch {
    ...
    fun closeCenter(doOnEnd: () -> Unit) {
        scope.launch {
            val spring = tween<Float>(400)
            launch {
                scale.animateTo(1f, spring)
            }
            cardMainScale.animateTo(0f, spring)
            doOnEnd()
            launch {
                cardMainScale.snapTo(1f)
            }
            returnCenter()
        }
    }
